...
application<- fread("./g_application_2012_2021.csv")
...
server <- function(input,output,session) {
  ...
  observeEvent(input$generate_competitive_positioning, {
    ...
    # dt <- cpc %>% filter(grepl(pattern = paste(input$market_cpcs_input, sep = "", collapse = "|"), x = cpc$patent_codes, ignore.case = TRUE))
    dt <- cpc %>% filter(grepl(pattern = paste(input$market_cpcs_input, sep = "", collapse = "|"), x = cpc$cpc_group, ignore.case = TRUE))
